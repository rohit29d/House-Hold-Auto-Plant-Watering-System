...
#else
const unsigned long TON_MS = 150000UL;          // 2 minutes and 30 seconds
...
void setup() {
  ...
  Serial.print(F("Mode       : "));
  Serial.println(TEST_MODE ? F("TEST") : F("PRODUCTION"));
  Serial.print(F("Relay      : "));
  Serial.println(RELAY_ACTIVE_HIGH ? F("Active HIGH") : F("Active LOW"));
  Serial.print(F("TON  (ms)  : "));
  Serial.println(TON_MS);
  Serial.print(F("TOFF (ms)  : "));
  Serial.println(TOFF_MS);
  Serial.print(F("MIN_DAY(ms): "));
  Serial.println(MIN_DAY_MS);
  Serial.print(F("MAX_ON (ms): "));
  Serial.println(MAX_VALVE_ON_MS);
  Serial.print(F("FALLBACK ms: "));
  Serial.println(FALLBACK_MS);
  Serial.print(F("CONFIRM ms : "));
  Serial.println(CONFIRM_MS);
  Serial.println(F("========================================="));
}
...
void loop() {
  ...
  if (dayState == NIGHT && solarVal >= THRESH_DAY)
    solarSuggest = DAY;
  if (dayState == DAY && solarVal <= THRESH_NIGHT)
    solarSuggest = NIGHT;
    ...
    if (!pendingTransition || pendingState != solarSuggest) {
      ...
      if (dayState == DAY) {
        ...
        if (virtualDay) {
          ...
          Serial.println(F(
              "[NIGHT] Virtual day ended — awaiting next fallback or sunrise"));
        } else {
...
WaterPhase computePhase(unsigned long now) {
  ...
  if (elapsed < MIN_DAY_MS)
    return PHASE_WARMUP;

  unsigned long schedElapsed = elapsed - MIN_DAY_MS;
  if (schedElapsed < SCHED_S1_END)
    return PHASE_SESSION1;
  if (schedElapsed < SCHED_GAP_END)
    return PHASE_GAP;
  if (schedElapsed < SCHED_S2_END)
    return PHASE_SESSION2;
  return PHASE_IDLE;
}
...
void updatePhase(WaterPhase newPhase) {
  ...
  switch (waterPhase) {
  case PHASE_WARMUP:
    Serial.println(F("Warmup — waiting for stable daylight"));
    break;
  case PHASE_SESSION1:
    Serial.println(F("Session 1 ON"));
    break;
  case PHASE_GAP:
    Serial.println(F("Gap — valve OFF between sessions"));
    break;
  case PHASE_SESSION2:
    Serial.println(F("Session 2 ON"));
    break;
  case PHASE_IDLE:
    Serial.println(F("Idle — schedule complete for today"));
    break;
  case PHASE_NIGHT:
    Serial.println(F("Night — valve OFF"));
    break;
  }
}
...
void printSolarDebug(int solarVal, unsigned long now) {
  ...
  case PHASE_WARMUP:
    ...
    break;
  case PHASE_SESSION1:
    Serial.println(F("SESSION1"));
    break;
  case PHASE_GAP:
    Serial.println(F("GAP"));
    break;
  case PHASE_SESSION2:
    Serial.println(F("SESSION2"));
    break;
  case PHASE_IDLE:
